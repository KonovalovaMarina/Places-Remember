...
class ViewsTestCase(TestCase):
    ...
    def test_get_home_status_code(self) -> None:
        self.client.force_login(self.user)
        response = self.client.get('/home/')
        self.assertEqual(response.status_code, 200)

    def test_get_home_empty_list_memories(self) -> None:
        self.client.force_login(self.user)
        response = self.client.get('/home/')
        self.assertEqual(str(response.context['memories'][0]),
                         ("User: First name 1 Last name 1, "
                          "Location: Екатеринбург, Тургенева 4, Title: Название1, "
                          "Description: Описание1"))

    def test_add_memory_status_code(self) -> None:
        self.client.force_login(self.user)
    ...
    def test_add_memory(self) -> None:
        ...
        self.client.post('/add_memory/', data=data)
    ...
    def test_delete_memory_status_code(self) -> None:
        self.client.force_login(self.user)
    ...
    def test_delete_memory(self) -> None:
        self.client.force_login(self.user)
        memory = Memory.objects.get(user=self.user)
        self.client.get('/delete_memory/' + str(memory.id))
